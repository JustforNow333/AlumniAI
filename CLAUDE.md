...
- Tech scope split: `query_scope: technical_role` requires an explicit technical title. Broad tech can count explicit technical titles or strong known-tech employers, but consulting/professional-services employers, marketplace-operations rows, finance-context data roles, and explicitly marketing/advertising roles at tech companies are not treated as tech direct matches.
...
Operation results carry `is_filtered`, top-level counts (`total_rows`, `raw_match_count`, `matched_row_count`, `returned_row_count`, `scored_result_count`, `displayed_count`, `display_limit`), and a `metrics` dict that duplicates them plus backward-compatible aliases (`rows_matched`, `rows_returned`, `searched_columns`). For people filters, `rows` should contain the full scored direct-match set while `displayed_count` tracks the UI cap. Search operations keep `search_columns` separate from `display_columns` — searched-only columns are not displayed unless the user asked for them. Display limits must never be presented as the answer count.
