...
22. Saved insights: manually saved answer snapshots tied to a dataset
   - Scope guard: insights are saved **manually** by the user via a Save insight button or from a History item. `/api/ask` never writes insights; automatic analysis history is implemented separately in `history_store.py` and `history_routes.py`.
...
24. History feature: automatic analysis log and full response reopening
   - Backend store (`backend/app/services/history_store.py`): metadata-only JSON registry at `backend/data/history.json` (config key `HISTORY_REGISTRY_PATH`, defaulting under `DATA_FOLDER`). History entries use UUID `history_id`, question-derived `title`, `dataset_id`, dataset filename snapshot, `question`, `answer_text`, exact JSON-safe `response_payload`, `status: "success"`, timestamps, and metadata such as row/column counts and searched columns. Atomic tmp-file writes match the dataset/insight stores. Missing registry files return an empty history, non-dict old records are skipped, and old entries with missing/non-object `response_payload` return `response_payload: null` instead of crashing.
   - Backend routes (`backend/app/routes/history_routes.py`, registered in `app/__init__.py`): `GET /api/history` newest-first with insertion-order tie breaks, `GET /api/history/<id>`, `POST /api/history`, `DELETE /api/history/<id>`, `DELETE /api/history` to clear all, and `POST /api/history/<id>/save-insight` as a backend bridge to saved insights. Clean JSON 400/404/500 errors follow the existing store pattern.
   - Ask integration (`backend/app/routes/chat_routes.py`): after `/api/ask` produces a successful non-empty operation-backed answer, the backend stores a history item with the same response-payload fields the frontend uses for rich rendering (`dataset_id`, `question`, `answer`, `answer_text`, `operation`, `result`, `analysis_intent`, `analysis_plan`, `operation_results`). Validation errors, missing datasets, planner failures, operation errors, and empty answers do not create history. History persistence failures do not break the ask response.
   - Frontend adapter (`frontend/api.jsx`): added `Alumni.history()`, `Alumni.historyItem(id)`, `Alumni.createHistoryItem(payload)`, `Alumni.deleteHistoryItem(id)`, `Alumni.clearHistory()`, and `Alumni.saveHistoryAsInsight(item)`. History entries normalize stored payloads through the same structured-answer sanitizer used by saved insights. Saving a history item as an insight sends the stored snapshot to `POST /api/insights` without re-running `/api/ask`.
...
27. Security test audit and offline security coverage
   - Audited the recently added security-related pytest coverage and kept the tests that exercise real boundaries: app headers/configuration, debug route gating, dataset file path containment, display sanitization, and offline model-call behavior.
   - Removed redundant granularity in `backend/tests/test_dataset_store.py` by consolidating five one-assert `get_file_type` tests into a single parametrized test.
   - Fixed the dataset absolute-path containment test to use a platform-real path outside the configured upload/data roots, so it works under the Windows backend venv as well as normal Python path handling.
   - Added `test_resolve_relative_traversal_stays_in_upload_root` to cover relative traversal-style registry paths without touching `eval` or generated-code behavior.
   - Added `backend/tests/test_app_security.py` for offline app security behavior: required security headers on API responses, `FLASK_SECRET_KEY` configuration, and `GET /api/debug/classify-row` returning the normal JSON 404 unless `FLASK_DEBUG` is enabled.
   - Strengthened `backend/tests/test_analysis_executor.py` so assumptions tests verify assumption propagation/ignoring behavior instead of only asserting that preview still succeeds.
   - Validation: `./backend/venv/Scripts/python.exe -m pytest backend/tests backend/evals/tests -q` passes 480 tests; `node --test frontend/tests/*.mjs` passes 46 tests.

28. Taxonomy-layer classification hardening
   - Hardened the existing taxonomy/classifier stack rather than adding a parallel architecture. The `/api/ask` row-selection path remains `classify_people_question` / intent inference -> people filter plan -> `_people_filter_result` -> `people_classifier.classify_candidate` -> presenter; the presenter never chooses which alumni belong in a result.
   - Expanded `industry_taxonomies.py` with first-class `investment_banking`, `marketing`, `operations`, and `government_legal` taxonomies, plus education service-program signals (`Teach For America`, `Corps Member`, `Program Coordinator`) needed for count-only education evals.
   - Split investment banking from broad banking. Investment banking now requires explicit investment-banking title wording; known banks alone or generic analyst/associate titles are uncertain or non-matches. Broad banking remains separate from finance and accepts known-bank rows for banking queries, including missing-title rows, while rejecting World Bank/government rows and finance-only equity research rows.
   - Added query-spec support for `excluded_industries`, `excluded_functions`, and `query_scope` (`industry`, `technical_role`, `subindustry`, `industry_exclusion`). Finance queries now separate plain finance from banking/investment-banking rows, and finance exclusion questions remove direct matches for excluded industries without letting weak adjacent evidence remove rows.
   - Added dedicated policies in `people_classifier.py` for tech, investment banking, banking, finance, marketing, operations, government/legal, and consulting. Labels remain `direct_match`, `adjacent`, `uncertain`, and `non_match`; only direct matches count by default, adjacent/uncertain never count unless a query explicitly asks for adjacent rows, and confidence cannot promote a non-direct label.
   - Tightened tech precision: technical-role scope requires an explicit technical title; broad tech can still count strong known-tech employers, but consulting/professional-services employers, marketplace-operations rows, finance-context data roles, and explicitly marketing/advertising roles at tech companies no longer count as tech. `known_tech_companies.json` remains the extension point and now includes `DigitalOcean` and `HubSpot`.
   - Added robust marketing, operations, and government/legal policies. Marketing distinguishes explicit marketing titles from generic growth/product/community wording; operations requires operations/supply-chain/logistics/manufacturing/revenue-management context; government/legal requires legal/policy/government titles or direct government/legal employers without pulling in weak public/education/healthcare/finance wording.
   - Fixed `operation_results`/eval count consistency: people-filter `rows` now contain the full direct-match set for scoring, while `displayed_count` remains the UI cap. Metrics/extras now include `direct_match_count`, `scored_result_count`, `final_display_count`, `adjacent_not_counted_count`, `uncertain_not_counted_count`, `excluded_count`, `candidate_count`, `target_industries`, `excluded_industries`, `taxonomy_used`, `display_columns`, and `classification_version`.
   - Updated eval scoring so full scored rows and display caps are checked separately; display limits no longer create false count mismatches when `operation_results` includes all direct matches.
   - Added/updated focused tests in `backend/tests/test_people_classifier.py`, `backend/tests/test_industry_taxonomies.py`, `backend/tests/test_analysis_toolkit.py`, and `backend/tests/test_ask_analysis_flow.py` for tech industry vs technical-role scope, strict investment banking, finance/banking exclusions, marketing/growth, operations, government/legal, consulting non-regression, adjacent/uncertain non-counting, and full scored-row returns.
   - Validation: focused offline industry eval improved to 21/21; full offline eval improved from the prior 35/55 baseline to 55/55 with zero hallucinations; hybrid-local (AI client forced off because the configured live hybrid path blocked on the `.env` OpenAI client) passes 55/55 and focused hybrid-local industry passes 21/21. `./backend/venv/Scripts/python.exe -m pytest backend/tests backend/evals/tests -q` passes 497 tests; `node --test frontend/tests/*.mjs` passes 46 tests.

29. Hybrid finance/banking routing fix (original-question re-classification)
   - Symptom: the live GPT-5.4 (hybrid) eval was 51/55 while offline was 55/55. The four failures were all finance/banking: `banking_002` ("Find alumni in banking.") returned 38 instead of 62, `banking_003` ("Which alumni work at banks?") returned a wrong 70-row set, `finance_001` ("Show me alumni in finance but not banking.") returned 62 banking rows, and `finance_002` ("Find finance alumni outside investment banking.") returned a 38-row IB-ish subset.
   - Root cause (not the taxonomy): `_plan_find_records` re-routes confident model `find_records` plans through `classify_people_question`, but it classified the model's *rephrased* `user_goal` rather than the original question. The model rewrote "in banking" → "…banking or investment banking" (routed to `investment_banking`), "work at banks" → "…a bank or financial institution" (classified to nothing → raw keyword search), "finance but not banking" → "…excluding banking" (the literal word "banking" won the industry guess), and "finance outside investment banking" → IB. Offline never hit this because `heuristic_intent` sets `user_goal` to the verbatim question.
   - Fix (surgical, finance/banking-safe but general): `infer_analysis_intent` now stamps `intent["original_question"]` with the verbatim question for every return path; `_plan_find_records` and the clarification path in `intent_to_analysis_plan` classify `original_question or user_goal`. This makes hybrid routing identical to offline for people/industry/exclusion questions without touching tech/consulting/marketing/operations/education/healthcare/government-legal, exact filters, messy employer matching, or display logic.
   - Precision tightening: added the same government/legal guard the banking policy already had to `_classify_for_finance`, so a development/world bank ("World Bank") with a generic title is no longer counted as finance. This brought `finance_001`/`finance_002` to exactly the 65-row finance set (was 70 with five "World Bank" Legal/Government rows leaking in under the precision threshold).
   - Tests: added `backend/tests/test_analysis_intent.py` regressions proving a misleading model `user_goal` cannot distort routing (banking stays broad banking, "work at banks" → banking, "finance but not banking" keeps finance + banking/IB exclusions, "finance outside IB" keeps finance + IB exclusion); added `backend/tests/test_analysis_toolkit.py` execution regressions for broad banking (titles + employers, not only IB), narrow investment banking, finance exclusion returning finance rows with banking/IB removed (and `operation_results.rows` equal to the final included set, never the excluded/candidate rows), and the World Bank-is-not-finance guard.
   - Validation: full backend suite `./backend/venv/Scripts/python.exe -m pytest -q` passes 505 (+1 opt-in eval node skipped); focused hybrid evals (`--case-id banking_001/002/003 finance_001/002`) pass 5/5 — `banking_001` still 18 investment-banking matches (runs deterministically, model_calls disallowed), `banking_002`/`banking_003` both return the 62-row broad banking set, `finance_001`/`finance_002` both return the 65-row finance set. Expected full hybrid eval: 55/55.

30. High-recall broad-tech discovery classification
   - Baseline before the change: with OpenAI disabled, the real broad-tech query "What alumni work in tech either as software engineers or other roles in a tech company?" returned 45 direct rows, 13 adjacent, and 11 uncertain internally, but only surfaced the 45 direct rows. Clear non-engineering employees at known tech companies and clear technical roles at non-tech/research/finance employers were under-surfaced.
   - Policy update: broad tech discovery now treats `role_is_technical OR employer_is_tech_company` as direct. Non-engineering roles at clearly tech employers (product, growth, strategy, finance, legal, sales, customer success, professional services, operations, government solutions, account management) are direct for broad "work in tech" and "tech company" queries. Strict software-engineer queries remain role-first and do not count product/business roles at tech companies as direct.
   - Taxonomy/classifier changes: expanded deterministic technical-title signals (SWE, founding/AI/research/database/client-solutions/forward-deployed/infrastructure/security/analytics engineering, database management systems, technical services/leads) and high-confidence tech employer coverage in `known_tech_companies.json` (OpenAI, Google, Twilio, Workday, InterSystems, ICEYE, Anduril, Arm, Epic, HubSpot, HackerRank, Indeed, Teads.tv, Chainalysis/Chainanalysis, Morph Systems, Cell Systems Logic, and other dataset-specific companies). Employer normalization now handles parentheticals, slash suffixes, typos/variants such as Capitol One, MIT Lincoln labratory, Chainanalysis, Indeed.com, Cogni Dao, and Rune technologies.
   - Query-scope behavior: `classify_people_question` distinguishes broad tech, tech-company, software-engineer-only, and software-engineer-in-finance prompts. Broad tech direct includes technical roles anywhere plus any role at clear tech companies; tech-company prompts are employer-first; strict technical-role prompts require implementation/engineering wording; software-engineers-in-finance allows technical finance rows while excluding finance business roles.
   - Response shape: people-filter results now keep backward-compatible `rows` as direct rows and add `direct_rows`, `adjacent_rows`, `uncertain_rows`, `direct_count`, `adjacent_count`, `uncertain_count`, `total_considered`, `surfaced_count`, and `row_sections`. Broad discovery answers render three surfaced sections: "Direct matches", "Adjacent tech-related matches", and "Uncertain possible matches". Adjacent/uncertain rows are useful review candidates, not counted direct matches, and internal classifier fields remain hidden by default.
   - Eval revamp: `backend/evals/scoring.py` now scores structured bucket expectations (`must_include_direct`, `must_include_surfaced`, `acceptable_categories`, `must_not_include_direct`, `required_sections`, `forbidden_columns_unless_asked`, `required_columns_if_available`, `count_must_equal_direct_rows`, and `expected_query_scope`) instead of brittle prose or exact-count snapshots. New failure categories separate row selection, bucket classification, count consistency, presentation, query-intent, and hallucination issues.
   - Tests: added/updated classifier, taxonomy parser, ask-flow, answer-schema, saved-insight, frontend adapter, and eval-scoring tests for broad-tech high recall, strict software-engineer behavior, tech-company employer-first behavior, finance/software overlap, direct-vs-adjacent buckets, count consistency, no duplicate/hallucinated rows, hidden debug/Major fields, LinkedIn preservation, and saved structured payload preservation. Existing saved insights remain immutable snapshots; new saved insights/history preserve the full structured response payload.
   - Validation snapshot after the change on the real 332-row alumni dataset: the broad-tech query returns 65 direct matches, 14 adjacent tech-related matches, and 12 uncertain possible matches, with all three sections surfaced. Direct examples include Amir Tavoli (OpenAI strategic finance), James Gladstone (Google product), Ryan Skinner (Twilio product), Matthew Fleischauer (Workday product), Kevin Vogt-Lowell (AI research engineer), Ashok Arun (founding engineer), Victor Bogert (database management systems), Neil Wusu, Dan Weissbard, Tal Shoer, Jordan Hauer, David Tuttle, Gregory Kallman, Tyler Bisk, Mason Hall, and Christopher Lombardo. Adjacent/review examples include Juan Gomez, Tomas Engquist, Richard Frost, Elliott Moreno, and Vincent Birmingham; Christopher Long is surfaced as a direct known-tech/company match under the updated taxonomy. Final validation: `./backend/venv/Scripts/python.exe -m pytest -q` passes 530 tests (+1 skipped), `node --test frontend/tests/*.mjs` passes 47 tests, offline eval passes 58/58, hybrid eval passes 58/58, classifier-live passes 14/14, and smoke-live passes 7/7.

31. Conservative repository bug and redundancy audit
   - Reproduced and fixed four backend regressions present at the start of the audit: model-rephrased banking/finance goals could override the original question, and tech-company queries could count technical roles at non-tech employers. `infer_analysis_intent` now preserves the verbatim question through validation/planning, and deterministic tech scopes (`industry`, `tech_company`, and `technical_role`, including finance intersections) reach the strict classifier without discarding the richer tech retrieval concepts.
   - Restored explicit people-result column selection without weakening default displays. Requests such as "names and employers only" now omit unrequested occupation/LinkedIn columns, while negated optional fields such as "Do not include majors" remain excluded. Focused unit coverage and the display eval cases guard both paths.
   - Hardened dataset persistence: failed registry writes after an upload now remove the newly saved orphan file; unsafe/legacy registry paths are reported as `status: missing` instead of crashing dataset listing; deleting such entries removes only their metadata and never touches a file outside configured storage roots.
   - Hardened response sanitization so malformed non-list column payloads are ignored safely and duplicate display columns are removed without misaligning list-row values.
   - Removed only verified redundancy: answer presentation now uses the shared OpenAI response/JSON parsing helpers and shared warning formatter; intent/toolkit pass-through text helpers were replaced by direct aliases; duplicate concept search terms and an impossible history payload type check were removed. Similar warning and persistence helpers with materially different behavior were intentionally left separate.
   - Added seven focused regression tests across `test_analysis_intent.py`, `test_dataset_store.py`, and `test_display_sanitizer.py`. Final validation: `./backend/venv/Scripts/python.exe -m pytest -q` passes 537 tests (+1 opt-in live test skipped), `node --test frontend/tests/*.mjs` passes 47 tests, `python3 -m compileall -q backend/app backend/evals backend/tests` passes, and the full offline product eval passes 58/58 with zero model calls.
