...
def test_read_unsupported_type(ctx):
    ensure_storage_dirs()
    path = get_storage_paths()["upload_folder"] / "test.txt"
...
def test_read_missing_file(ctx):
    ensure_storage_dirs()
    missing = get_storage_paths()["upload_folder"] / "does_not_exist.csv"
    with pytest.raises(DatasetFileMissingError, match="missing"):
        read_dataframe_from_path(str(missing), "csv")


def test_read_empty_csv(ctx):
    ensure_storage_dirs()
    csv_path = get_storage_paths()["upload_folder"] / "empty.csv"
    csv_path.write_text("")
